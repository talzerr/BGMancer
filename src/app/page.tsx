      ...
      <header className="sticky top-0 z-40 border-b border-white/[0.06] bg-zinc-950/80 backdrop-blur-xl">
        <div className="mx-auto flex h-14 max-w-6xl items-center justify-between px-4 sm:px-6">
          ...
          <div className="flex items-center gap-2.5">
            ...
            <div>
              <h1 className="text-sm leading-none font-bold tracking-tight text-white">BGMancer</h1>
            ...
            {AUTH_CONFIGURED ? (
              session?.user ? (
                <>
                  ...
                  <span className="hidden text-sm text-zinc-400 sm:block">{session.user.name}</span>
                  <form
                    action={async () => {
                      "use server";
                      await signOut();
