...
beforeEach(() => {
  ...
  gameId = seedTestGame(db, userId, { id: "game-vt" });
  trackNames = seedTestTracks(db, gameId, 3);
});
...
describe("VideoTracks", () => {
  describe("upsertBatch", () => {
    ...
    it("should update track_name on conflict", () => {
      VideoTracks.upsertBatch([{ videoId: "v1", gameId, trackName: trackNames[0] }]);
      VideoTracks.upsertBatch([{ videoId: "v1", gameId, trackName: trackNames[1] }]);
    ...
    it("should be a no-op when given an empty array", () => {
      ...
      const rows = db.prepare("SELECT * FROM video_tracks WHERE game_id = ?").all(gameId);
      expect(rows).toHaveLength(0);
    });

    it("should allow null track_name", () => {
      VideoTracks.upsertBatch([{ videoId: "v-null", gameId, trackName: null }]);
  ...
  describe("upsertSingle", () => {
    it("should create a new video track entry", () => {
      VideoTracks.upsertSingle(gameId, trackNames[0], {
        videoId: "v-single",
        durationSeconds: 180,
        viewCount: 5000,
      });
    ...
    it("should preserve existing duration via COALESCE on update", () => {
      VideoTracks.upsertSingle(gameId, trackNames[0], {
        videoId: "v-coalesce",
        durationSeconds: 200,
        viewCount: 1000,
    ...
    it("should preserve existing view_count via COALESCE when new value is null", () => {
      VideoTracks.upsertSingle(gameId, trackNames[0], {
        videoId: "v-vc",
        durationSeconds: 100,
        viewCount: 3000,
    ...
    it("should update track_name on conflict", () => {
      VideoTracks.upsertSingle(gameId, trackNames[0], { videoId: "v-rename" });
      VideoTracks.upsertSingle(gameId, trackNames[1], { videoId: "v-rename" });
  ...
  describe("storeDurations", () => {
    it("should write duration and view_count for new entries", () => {
    ...
    it("should not overwrite existing duration (write-once via COALESCE)", () => {
      VideoTracks.storeDurations([
    ...
    it("should always refresh view_count", () => {
      VideoTracks.storeDurations([
  ...
  describe("getByGame", () => {
    it("should return a map with correct structure", () => {
      VideoTracks.upsertBatch([
        { videoId: "v1", gameId, trackName: trackNames[0] },
      ...
      const map = VideoTracks.getByGame(gameId);
    ...
    it("should return an empty map for an unknown game", () => {
      const map = VideoTracks.getByGame("nonexistent-game");
  ...
  describe("getTrackToVideo", () => {
    it("should return only entries with non-null track_name", () => {
      VideoTracks.upsertBatch([
        { videoId: "v1", gameId, trackName: trackNames[0] },
        { videoId: "v2", gameId, trackName: null },
      ...
      const map = VideoTracks.getTrackToVideo(gameId);
    ...
    it("should return an empty map for a game with no video tracks", () => {
      const map = VideoTracks.getTrackToVideo(gameId);
    ...
    it("should not return entries from a different game", () => {
      const otherGameId = seedTestGame(db, userId, { id: "game-vt-other" });
      const otherTracks = seedTestTracks(db, otherGameId, 1);
