...
Three mechanisms apply, in order of strength:

1. **Same-Game Exclusion.** When selecting a track for a slot, the candidate search first excludes all tracks from the same game as the previous slot's track. Only if no qualifying candidate survives this exclusion does it fall back to a relaxed pass that allows same-game placement. This is a soft constraint: it prevents consecutive same-game tracks when alternatives exist, but does not leave a slot empty to avoid repetition.
